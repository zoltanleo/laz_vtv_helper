...
type
  ...
  private
    // --- УДАЛЕНА ВСПОМОГАТЕЛЬНАЯ ПРОЦЕДУРА CountNodesAndChildren ---
...
var
  ...
  NodeData: PMyRecord;
  // --- УБРАНО: переменная ChildCount теперь не нужна для подсчёта ---
  // ChildCount: Integer;
  // --- КОНЕЦ УБРАНО ---
...
begin
  ...
  // Для подсчёта используем обход дерева, начиная с RootNode или всех корней
  TotalNodeCount := 0;
  if Assigned(RootNode) then
  begin
    // Подсчёт узлов в поддереве, начиная с RootNode
    Node := RootNode;
    while Assigned(Node) do
    begin
      Inc(TotalNodeCount);
      // Переход к следующему узлу в порядке обхода
      if (ATree.ChildCount[Node] > 0) and Assigned(Node.FirstChild) then
        Node := Node.FirstChild
      else
      begin
        while Assigned(Node) and not Assigned(Node.NextSibling) do
          Node := Node.Parent;
  ...
  begin
    ...
    // --- ИЗМЕНЕНО: используем ATree.ChildCount[Node] вместо ручного подсчёта ---
    AStream.Write(ATree.ChildCount[Node], SizeOf(Cardinal));
    // --- КОНЕЦ ИЗМЕНЕНИЯ ---
...
var
  ...
  NodeData: PMyRecord;
  // --- ИЗМЕНЕНО: переменная для хранения количества детей теперь Cardinal ---
  ChildCount: Cardinal;
  // --- КОНЕЦ ИЗМЕНЕНИЯ ---
...
begin
  ...
  begin
    ...
    begin
        ...
        for i := 0 to pred(Integer(ChildCount)) do // --- ИЗМЕНЕНО: приведение к Integer для цикла ---
        begin
